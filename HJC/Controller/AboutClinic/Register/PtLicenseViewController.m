...
#import "MebLicenseViewController.h"

@interface PtLicenseViewController ()
...
@implementation PtLicenseViewController
...
-(NavView *)navView{
    if(!_navView){
        ...
        navView.titleLabel.text = @"药品经营许可证上传";
...
-(void)toolButtonAction:(UIButton*)sender{
    ...
    [BaseApi postMenthodWirhUrlString:[NSString stringWithFormat:@"%@%@?Corpid=%@&P_type=%@",HostPath,UploadClinicImages,self.corpid,@"3"] paramDic:pargrams image:self.businessLicense.image progress:^(NSProgress * upload) {
